...
> **Nota de ejecución (2026-09-18):** el módulo se repartió en tres ficheros en vez de uno.
> La regla `react/only-export-components`, que `.oxlintrc.json` activa a propósito, avisa en
> cualquier fichero que exporte a la vez un componente y funciones, incluso a través de un
> `export *`. Con el proveedor en su propio fichero y `index.ts` como barril puro, el lint
> queda limpio. La ruta de import pública no cambia: todo se sigue importando de `../i18n`.
...
| `src/i18n/core.ts` | `interpolate`, `translate`, `detectLang`, los contextos y los hooks `useLang`, `useSetLang`, `useT`. Sin componentes. |
| `src/i18n/LanguageProvider.tsx` | Solo el componente `LanguageProvider`. |
| `src/i18n/index.ts` | Barril: re-exporta `core` y `LanguageProvider`. Los consumidores importan siempre de `../i18n`. |
