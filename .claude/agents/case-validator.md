...
3. Para cada elemento de `murders`, en orden, con su `index`:
   ...
   - **Coordenadas y país**: `bash pipeline/geocode.sh "<city>, <country>"` es obligatorio, una vez por ciudad distinta del candidato; si el candidato trae `region` no nula, la consulta es `bash pipeline/geocode.sh "<city>, <region>, <country>"`; si devuelve `NOT FOUND`, prueba `"<region>, <city>, <country>"` (barrios y distritos) y, si tampoco, `"<city>, <country>"`; usa el primer resultado y anota en `notes` cuál fue; y además esa `region` (o su forma en el idioma de la fuente) tiene que aparecer en la fuente asociada a esa ciudad o a ese asesinato, o `ok: false` con error `región sin respaldo`; no te fíes de tu memoria ni del `geocode` que trae el fichero de evidencias. Con su resultado, `bash pipeline/distance.sh <lat> <lng> <latRef> <lngRef>`: si supera 30 km, `ok: false` con error `coordenadas a X km de la ciudad`. Mira además el `display_name` que devuelve el script: si el país o la región que trae no es el `country` del candidato ni la región que la fuente asocia a esa ciudad, es una ciudad homónima de otro país y va `ok: false` con error `ciudad homónima: geocodifica en <display_name>`. Si la fuente sitúa la ciudad en un país distinto del que escribe el candidato, `ok: false` con error `país incorrecto`.
   - **Idioma**: `city`, `region`, `country`, `method`, `summary` y `activeYears` van en español (regla 9 del generador). Si `method` o `summary` contienen palabras inglesas evidentes (`stabbed`, `strangled`, `killed`, `murdered`, `shot`, `beaten`, `mutilated`, `the`, `and`, `with`) o `city`/`country` usan la forma inglesa cuando existe una española conocida (`Rostov-on-Don`/`Rostov del Don`, `Russia`/`Rusia`, `New York`/`Nueva York`), `ok: false` con error `texto no en español`. Las citas (`sourceQuote`) y los nombres propios de víctimas se dejan como en la fuente.
